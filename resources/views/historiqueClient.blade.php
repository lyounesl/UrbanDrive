...
@section('title', 'Historique des Trajets - Urban Drive')
...
<div class="historique-container">
    ...
    <div class="historique-header">
        ...
        <div class="header-subtitle">{{ Auth::user()->name }} — Tous vos trajets en un coup d'œil</div>
        <div class="header-actions">
            <a href="/client/{{ $num }}" class="btn-white">← Retour à l'espace client</a>
            <a href="/client/{{ $num }}/reservation" class="btn-blue">🚗 Nouvelle réservation</a>
        </div>
    </div>
    ...
    <div class="historique-content">
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-number">{{ $totalTrajets }}</div>
            ...
            <div class="stat-card">
                <div class="stat-icon">💶</div>
                <div class="stat-number">{{ number_format($totalDepense, 2, ',', ' ') }} €</div>
        ...
        <div class="section-title">
            Derniers trajets
            <span class="trajets-count">{{ $totalTrajets }} trajet(s)</span>
        </div>

        @forelse($historique as $trajet)
            <div class="trajet-card" data-statut="{{ $trajet->statut }}">
                ...
                <div class="trajet-top">
                    <div>
                        ...
                        <div class="trajet-id">#{{ $trajet->id }}</div>
                    </div>
                    <div class="trajet-statut statut-{{ $trajet->statut }}">
                ...
                <div class="trajet-route">
                    <div class="lieu">
                        ...
                        <div class="lieu-nom">{{ $trajet->depart }}</div>
                    ...
                    <div class="lieu">
                        ...
                        <div class="lieu-nom">{{ $trajet->destination }}</div>
                        <div class="lieu-horaire">
                ...
                {{-- Infos bas : badges + prix + boutons --}}
                <div class="trajet-infos">

                    {{-- Badges gauche --}}
                    <div style="display:flex; gap:1.5rem; flex-wrap:wrap; align-items:center;">
                        <div class="info-badge">
                            <span>🚙</span> {{ ucfirst($trajet->vehicule_type) }}
                        </div>
                        @if($trajet->distance_km)
                            <div class="info-badge">
                                <span>📏</span> {{ $trajet->distance_km }} km
                            </div>
                        @endif
                        @if($trajet->chauffeur)
                            <div class="info-badge">
                                <span>👤</span> {{ $trajet->chauffeur->name }}
                            </div>
                        @endif
                        @if($trajet->note_chauffeur)
                            <div class="trajet-note">
                                @for($i = 1; $i <= 5; $i++)
                                    <span class="{{ $i <= $trajet->note_chauffeur ? 'star-filled' : 'star-empty' }}">★</span>
                                @endfor
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="trajet-boutons">
                        <div class="trajet-prix">{{ number_format($trajet->prix, 2, ',', ' ') }} €</div>

                        @if($trajet->statut === 'en_attente' && $trajet->reservation_id)
                            <form method="POST"
                                  action="{{ route('client.annuler', ['num' => $num, 'reservation' => $trajet->reservation_id]) }}"
                            ...
                            <form method="POST"
                                  action="{{ route('client.demarrer', ['num' => $num, 'historique' => $trajet->id]) }}"
                        ...
                        @if($trajet->statut === 'en_cours')
                            <form method="POST"
                                  action="{{ route('client.terminer', ['num' => $num, 'historique' => $trajet->id]) }}"
                        ...
                        @if($trajet->statut === 'terminee' && is_null($trajet->note_chauffeur))
                            <a href="{{ route('client.avis', ['num' => $num]) }}" class="btn-avis">
                                ⭐ Donner un avis
                            </a>
                        @endif
                    </div>
        ...
        @empty
            <div class="no-trajets">
                ...
                <a href="/client/{{ $num }}/reservation" class="btn-reserver">
